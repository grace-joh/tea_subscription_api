...
RSpec.describe "Cancel a customer's tea subscription" do
  before(:each) do
  ...
  describe "Happy path" do
    it 'creates a new customer tea subscription' do
      patch '/api/v0/subscriptions', headers: @headers, params: JSON.generate(@params)
      ...
      expect(subscr_attr[:status]).to eq('cancelled')
    end
  end

  describe "Sad path" do
    it 'returns an error if customer_id is invalid' do
      params = { customer_id: -1, tea_id: @assam_tea.id, subscription_id: @sub_3.id }
      ...
      expect(error[:detail]).to eq("Couldn't find CustomerTeaSubscription with [WHERE \"customer_tea_subscriptions\".\"customer_id\" = $1 AND \"customer_tea_subscriptions\".\"tea_id\" = $2 AND \"customer_tea_subscriptions\".\"subscription_id\" = $3]")
    end
  end
end
